...
import RelatedTopics from '../topic/related-topics'
...
const editUrl = (path: string) => `${siteMetadata.siteRepo}/blob/main/topics/${path}`
        ...
        <header className="pt-6 xl:pb-6">
          ...
          <div className="flex">
            ...
            <div className="mb-5">
              ...
              {content.summary && <Summary summary={content.summary} />}

            <div className="prose max-w-none pb-8 pt-10">{children}</div>
            <RelatedTopics post={content} />
            <footer>
              <div className="divide-gray-200 text-sm font-medium leading-5 dark:divide-gray-700 xl:col-start-1 xl:row-start-2 xl:divide-y">
                ...
                {(next || prev) && (
                    <div className="flex justify-between items-center py-4 xl:py-8">
                      {prev && prev.path && (
                        <PageLink dir="previous" title={prev.title} href={`/${prev.path}`} />
                      )}
                      {next && next.path && (
                        <PageLink title={next.title} href={`/${next.path}`} />
                      )}
                    </div>
                  )}
              </div>

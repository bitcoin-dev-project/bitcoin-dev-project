...
export default function Accordion({ content, title }: Props) {
  ...
  return (
    <div className="flex transition-all flex-col border rounded-lg border-[#D3D3D3] self-center w-[50%] lg:w-[80%]">
      <button
        type="button"
      ...
      >
        ...
        <svg
          className={`${iconStyle} transition-all ease-linear`}
          width="24"
          height="14"
          viewBox="0 0 24 14"
          fill="none"
      ...
      <p
        className={`${visibilityStyle} text-left mx-7 font-medium inline-block transition-[height,opacity] ease-linear`}
      >
        {content}
      </p>
    </div>
